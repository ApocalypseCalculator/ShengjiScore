...
export function getWinningPlayerIdx(players: Player[]): number {
    let winner = players[0];
    let winnerIdx = 0;
    let tied = false;
    players.forEach((player, idx) => {
        if (idx > 0) {
            if (player.round > winner.round || (player.round == winner.round && player.score > winner.score)) {
                winner = player;
                winnerIdx = idx;
                tied = false;
            }
            else if (player.round == winner.round && player.score == winner.score) {
                tied = true;
            }
        }
    });
    return tied ? -1 : winnerIdx;
}
